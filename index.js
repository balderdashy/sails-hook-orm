...
module.exports = function (sails) {
  ...
  var loadAppModules = require('./lib/load-app-modules')(sails);
  var initializeWaterline = require('./lib/initialize-waterline')(sails);
  var validateModelDef = require('./lib/validate-model-def')(sails);
  var validateDatastoreConfig = require('./lib/validate-datastore-config')(sails);
      ...
      async.auto({
        ...
        forceLoadAdapters: function(next) {
          _.each(sails.config.connections, function(connection, connectionId) {
            if (connection.forceLoadAdapter) {
              validateDatastoreConfig(connectionId, '<FORCE>');
            }
          });
          return next();
        },
        ...
        normalizedModelDefs: ['_loadModules', function normalizeModelDefs(next) {
          _.each(sails.models, function (model, identity) {
            validateModelDef(model, identity);
          });
        ...
        instantiatedCollections: ['_doubleCheckMigration', function (next, async_data) {
          ...
          initializeWaterline(async_data.normalizedModelDefs, next);
        }]
