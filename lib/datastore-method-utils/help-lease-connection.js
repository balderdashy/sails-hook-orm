  ...
  options.driver.getConnection({
    ...
    (function _makeCallToDuringFn(proceed){
        ...
        options.driver.releaseConnection({ connection: db, meta: options.meta }, {
          error: function(secondaryErr) {
            ...
            return done(new Error(
              'The code using this leased db connection encountered an error:\n'+
              '``` (1)\n'+
              duringErr.stack +'\n'+
              '```\n'+
      ...
      options.driver.releaseConnection({ connection: db, meta: options.meta }, {
        error: function(secondaryErr) {
          return done(new Error(
            'The code using this leased db connection ran successfully, but afterwards, '+
            'there was an issue releasing the connection:\n'+
            '```\n' +
            secondaryErr.stack+'\n'+
            '```'
          ));
