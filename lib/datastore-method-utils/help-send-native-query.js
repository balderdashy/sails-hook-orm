...
module.exports = function helpSendNativeQuery(options, done){
  ...
  (function _leaseConnectionMaybe(during, done){

    if (options.connection) {
      during(options.connection, function (err, rawResult){
    ...
    function _during(db, proceed){
      ...
      options.driver.sendNativeQuery({
        ...
        queryFailed: function (failureReport){
          ...
          // Sanity check.
          try {
            assert(_.isError(failureReport.error), 'The `error` property of the failure report from the low-level `sendNativeQuery` driver method should always be an Error instance, but this time, it is not!  (This indicates that there is a bug in this adapter/driver.)');
