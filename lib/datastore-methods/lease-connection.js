...
// TODO: move `checkAdapterCompatibility()` invocation into here

// TODO: extrapolate the `parley` stuff into the buildRDI() utility so that this is just a private helper

// TODO: instead, just pass in `options, done`
// `options.dsEntry`
//   => driver
//   => manager
//   => config
// `options.duringFn`
// `options.meta`
// `options.usingConnection` // << this is not actually in use for this particular method, but since this is a template for the others, including it temporarily.  It should be removed before calling this finished.
// etc

/**
 * helpLeaseConnection()
 *
 * Attempt to get a connection from the specified datastore's manager, run the
 * provided `during` function, and finally release the connection.
 *
 * > This utility is used to implement the `leaseConnection()` datastore (RDI) method.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 *                `during` function.
 */
return function helpLeaseConnection(options, done){

  // Check that this adapter is compatible w/ datastore methods in general,
  // as well as with this version of Sails / Waterline.  Also verify that
  // it implements all of the underlying methods which are mandatory for this
  // particular datastore method to function properly.
  try {
    checkAdapterCompatibility(options.datastoreName, options.adapter, [
      'getConnection',
      'releaseConnection'
    ]);
  } catch (e) { return done(e); }

  // Now grab the datastore entry from the adapter to simplify the code below.
  var dsEntry = adapter.datastores[datastoreName];
