 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * @returns {Deferred?} if no explicit callback provided
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
module.exports = function helpSendNativeQuery(options, explicitCbMaybe){
  ...
  return parley(function _handleExec(done){
    ...
    if (!_.has(options.driver, 'getConnection')) {
      return done(new Error('The provided driver for the ' + options.datastoreIdentity + ' datastore is missing a required `getConnection` method and therefore can\'t be used for this.'));
    }

    // If the driver doesn't have a sendNativeQuery method it's invalid and can't be
    // used by this function.
    if (!_.has(options.driver, 'sendNativeQuery')) {
      return done(new Error('The provided driver for the ' + options.datastoreIdentity + ' datastore is missing a required `sendNativeQuery` method and therefore can\'t be used for this.'));
    }
    ...
    if (!_.has(options.driver, 'parseNativeQueryError')) {
      return done(new Error('The provided driver for the ' + options.datastoreIdentity + ' datastore is missing a required `parseNativeQueryError` method and therefore can\'t be used for this.'));
    }

    // Validate the query exists after the deferred has been processed
    if (!_.has(options, 'nativeQuery')) {
      return done(new Error('Missing or invalid nativeQuery.'));
    }
    ...
    (function ensureConnection(proceed) {
      ...
      if (leasedExternally) {
        return setImmediate(function() {
          proceed(undefined, options.dbConnection);
        });
      }
      ...
      options.driver.sendNativeQuery({
        ...
        if (!leasedExternally) {
          options.driver.releaseConnection({
            ...
            if (err && releaseError) {
              ...
              return done(enhancedDuringError);
            }
            ...
            if (err) {
              // If there wasn't a code on it for some reason, add a bit of flaverr
              if (!err.code) {
                return done(flaverr({ code: 'error' }, parsedError));
              }
              // Otherwise continue on and pass the error through
              return done(parsedError);
            }

            // Otherwise just return the results of the native query.
            done(undefined, sendNativeQueryReport.result);
          });
        ...
        return done(undefined, sendNativeQueryReport.result);
