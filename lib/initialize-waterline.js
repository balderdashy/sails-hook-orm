...
module.exports = function howto_initializeWaterline(sails) {
   ...
   *              -> waterlineOrm
   */
  return function initializeWaterline(modelDefs, cb) {
    ...
    var waterlineOrm = new Waterline();
    _.each(modelDefs, function loadModelsIntoWaterline(modelDef, modelID) {
      ...
      waterlineOrm.loadCollection(Waterline.Collection.extend(modelDef));
    });
    ...
    waterlineOrm.initialize({
      ...
      // Success
      return cb(undefined, waterlineOrm);
    });
  };
};
