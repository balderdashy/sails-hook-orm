    ...
    _determineRelevantDatastoreNames: ['_mergeInProgrammaticModuleDefs', function (unused, next){
      ...
      try {
        ...
        _.each(sails.config.datastores, function _eachDatastoreConfig(datastoreConfig, datastoreName) {
          ...
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          // TODO: in the appropriate spot in the code where other datastore config is checked,
          // add compatibility error for the `forceLoadAdapter` datastore config (no longer supported)
          // (this also needs to make its way into the migration guide, and we need to search the
          // Sails docs for it and make sure we've changed the explanation accordingly.)
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -



          // As of Sails v1, all configured datastores are valid.
          // ================================================================================
          ...
          // }
          //
          // // If the datastore config is not in use by any models or app-wide defaults
          // // -AND- it does not have the `forceLoadAdapter` flag set to true, then ignore it.
    ...
    _validateDatastoreConfigsAndBuildAccessors: ['_attemptToLoadUnrecognizedAdapters', function (async_data, next){
      ...
      try {
        ...
        _.each(async_data._determineRelevantDatastoreNames, function _eachRelevantDatastoreName(datastoreName) {
          ...
          // Build our datastore instance
          // (a dictionary with public methods as well as other important state)
          var datastoreInstance = {};//TODO
          ...
          // for use in the hook.
          datastoreInstance.internalConfig = normalizedDatastoreConfig;

          // Store the adapter on the internal datastore
          datastoreInstance.adapter = adapter;
          ...
          // (TODO: remove this!)
          datastoreInstance.fooBar = require('./datastore-methods/foo-bar');

          //  ╦  ╔═╗╔═╗╔═╗╔═╗  ┌─┐┌─┐┌┐┌┌┐┌┌─┐┌─┐┌┬┐┬┌─┐┌┐┌
          //  ║  ║╣ ╠═╣╚═╗║╣   │  │ │││││││├┤ │   │ ││ ││││
          //  ╩═╝╚═╝╩ ╩╚═╝╚═╝  └─┘└─┘┘└┘┘└┘└─┘└─┘ ┴ ┴└─┘┘└┘
          // Add a wrapper for handling the .leaseConnection method
          datastoreInstance.leaseConnection = function(during, cb) {
          ...
          datastoreInstance.sendStatement = function(statement, dbConnection, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
          ...
          datastoreInstance.sendNativeQuery = function(nativeQuery, dbConnection, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
          ...
          datastoreInstance.transaction = function(during, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
          ...
          // Expose this datastore on `hook.datastores`.
          hook.datastores[datastoreName] = datastoreInstance;
      ...
      hook.getDatastore = function getDatastore(datastoreName) {
        ...
        if (foundDatastore) {
          ...
          return {
            ...
            fooBar: foundDatastore.fooBar,//<<TODO: remove
          };
