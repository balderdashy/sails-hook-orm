...
module.exports = function initialize(hook, sails, done){
  ...
  async.auto({
    ...
    _checkForGlobalMongoSettings: ['_validateDatastoreConfigsAndBuildAccessors', function (async_data, next) {
      ...
      try {
        ...
        // If no attribute exists, we're done (for now).  Each model will be checked individually
        // to ensure that if it's using Mongo, it has a correctly-configured PK attribute.
        if (!defaultPrimaryKeyAttr) {
          return next();
        }

        if (defaultAdapterIdentity === 'sails-mongo' && (defaultPrimaryKeyAttr.autoIncrement || defaultPrimaryKeyAttr.type !== 'string' || defaultPrimaryKeyAttr.columnName !== '_id')) {
