...
module.exports = function initialize(hook, sails, done){
  ...
  async.auto({
    ...
    //  └─  └  ┴└─└─┘┴ ┴  ┴ ┴┴  ┴    ┘└┘└─┘─┴┘└─┘────┴ ┴└─┘─┴┘└─┘┴─┘└─┘└─┘  └  └─┘┴─┘─┴┘└─┘┴└─  ─┘
    //
    // For every valid datastore config which is referenced by at least one model (or with `forceLoadAdapter` set, ensure its
    // referenced adapter is loaded. Note that we do not attempt to validate/normalize datastore configs or models here--
    // we simply ignore datastore configs that cannot be parsed.  The goal is to ensure adapters are loaded.
    //
    // If we find a not-yet-loaded adapter being referenced, attempt to require it from the `node_modules/` directory of this
    // Sails application.
    _attemptToLoadUnrecognizedAdapters: ['_loadModelsAndCustomAdapters', function (next){

      _.each(sails.config.connections, function _eachDatastoreConfig(datastoreConfig, datastoreIdentity) {

        // If the datastore config is not valid, then ignore it.
        if (!util.isObject(datastoreConfig) || !util.isString(datastoreConfig.adapter) || !datastoreConfig.adapter) {
          return;
        }
        ...
        var isDatastoreInUse =
        _.any(hook.models, function (modelDef) {
          // Check for "connection" specified as string.
          if (util.isString(modelDef.connection) && modelDef.connection === datastoreIdentity) { return true; }
          // Check for "connection" specified as array.
          else if (util.isArray(modelDef.connection) && util.isString(modelDef.connection[0]) && modelDef.connection[0] === datastoreIdentity) { return true; }
          else { return false; }
        });
        // Check for "connection" in app-wide defaults.
        var isDatastoreInAppWideDefaults = (util.isObject(sails.config.models) && sails.config.models.connection === datastoreIdentity);
        if (!isDatastoreInUse && !isDatastoreInAppWideDefaults && !datastoreConfig.forceLoadAdapter) {
          return;
        }

        // Now check if the referenced adapter has aready been loaded one way or another.
        if (util.isObject(datastoreConfig) && util.isString(datastoreConfig.adapter)) {
    ...
    _buildAndExposeDatastores: ['_attemptToLoadUnrecognizedAdapters', function (next){
      ...
      try {
        hook.datastores = _.reduce(sails.config.connections, function _eachDatastoreConfig(memo, datastoreConfig, identity) {
          // Expose a dictionary for this datastore.
          memo[identity] = {};
          ...
          // If no adapter exists (e.g. because this datastore is not in use), then go no further.
          if (!memo[identity].adapter) {
            return memo;
          }

          // Otherwise, we know the adapter is locked and loaded.  Err..and validated.  And normalized.

          // Expose a reference to relevant normalized adapter as `.adapter`:
    ...
    _normalizeModelDefs: ['_attemptToLoadUnrecognizedAdapters', function (next) {
      try {
