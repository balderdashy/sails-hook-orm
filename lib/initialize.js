...
module.exports = function initialize(hook, sails, done){
  ...
  async.auto({
    ...
    _determineRelevantDatastoreIdentities: ['_mergeInProgrammaticModuleDefs', function (unused, next){

      var relevantDatastoreIdentities = [];
      _.each(sails.config.datastores, function _eachDatastoreConfig(datastoreConfig, datastoreIdentity) {
        ...
        var isDatastoreInUse = _.any(hook.models, function eachModel(modelDef) {
          ...
          if (_.isString(modelDef.datastore) && modelDef.datastore=== datastoreIdentity) {
            return true;
          }
          // Check for "datastore" specified as array.
          else if (_.isArray(modelDef.datastore) && _.isString(modelDef.datastore[0]) && modelDef.datastore[0] === datastoreIdentity) {
            return true;
          }
          // Check for `undefined` datastore (i.e. indicating that the default should be used)
            ...
            var isDatastoreAppWideDefault = (_.isObject(sails.config.models) && sails.config.models.datastore === datastoreIdentity);
            if (isDatastoreAppWideDefault) {
              return true;
            }
          }//</model has no explicit "datastore" defined>

          // Otherwise, this model has something weird going on for its `datastore` property,
          else {
    ...
    _attemptToLoadUnrecognizedAdapters: ['_determineRelevantDatastoreIdentities', function (async_data, next){
      try {
    ...
    _validateDatastoreConfigsAndBuildAccessors: ['_attemptToLoadUnrecognizedAdapters', function (async_data, next){
      ...
      try {
        ...
        _.each(async_data._determineRelevantDatastoreIdentities, function _eachRelevantDatastoreIdentity(datastoreIdentity) {
          ...
          // for use in the hook.
          hook.datastores[datastoreIdentity].internalConfig = normalizedDatastoreConfig;

          // Store the adapter on the internal datastore
          hook.datastores[datastoreIdentity].adapter = adapter;
          ...
          hook.datastores[datastoreIdentity].leaseConnection = function(during, cb) {
          ...
          hook.datastores[datastoreIdentity].sendStatement = function(statement, dbConnection, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
          ...
          hook.datastores[datastoreIdentity].sendNativeQuery = function(nativeQuery, dbConnection, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
          ...
          hook.datastores[datastoreIdentity].transaction = function(during, cb) {
            // Validate the adapter API version and that the datastore is able
            // to run the lease connection as far as we can tell.
            try {
              validateDatastoreConnection({
      ...
      // by name.
      hook.datastore = function datastoreGetter(datastoreIdentity) {
        var foundDatastore = hook.datastores[datastoreIdentity];
        if (foundDatastore) {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'production') {
          ...
          var modelsUsingSailsDisk = _.reduce(hook.models, function(memo, normalizedModelDef, identity){
            ...
            var referencedDatastore = normalizedModelDef.datastore[0];
      ...
      try {
        _.each(async_data._buildOntology.collections, function _eachInstantiatedModel(wlModel, modelIdentity) {
          ...
          if (wlModel.datastore) {
            throw new Error('Consistency violation: Instantiated Waterline model (`'+modelIdentity+'`) already has a `datastore` property (`'+wlModel.datastore+'`).');
          }
          else {
            wlModel.datastore = hook.datastores[wlModel.datastore[0]];
          }
