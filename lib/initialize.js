    ...
    _determineRelevantDatastoreIdentities: ['_mergeInProgrammaticModuleDefs', function (next){
      ...
      _.each(sails.config.connections, function _eachDatastoreConfig(datastoreConfig, datastoreIdentity) {
        ...
        _.any(hook.models, function (modelDef) {
          ...
          if (_.isString(modelDef.connection) && modelDef.connection === datastoreIdentity) { return true; }
          // Check for "connection" specified as array.
          else if (_.isArray(modelDef.connection) && _.isString(modelDef.connection[0]) && modelDef.connection[0] === datastoreIdentity) { return true; }
          else { return false; }
        });
        // Check for "connection" in app-wide defaults.
        var isDatastoreInAppWideDefaults = (_.isObject(sails.config.models) && sails.config.models.connection === datastoreIdentity);
    ...
    _productionCheck: ['_normalizeModelDefs', function (next) {
      try {
        ...
        if (process.env.NODE_ENV === 'production') {
          ...
          console.log('datastoresUsingSailsDisk',datastoresUsingSailsDisk);
          ...
          console.log('modelsUsingSailsDisk',modelsUsingSailsDisk);
