...
module.exports = function validateModelDef (originalModelDef, modelIdentity, hook, sails) {
  ...
  _.each(originalModelDef.attributes, function deprecateInstanceMethods(val, attributeName) {
    ...
    if (_.isFunction(val)) {
      sails.log.debug('It looks like you are using an instance method on the ' + modelIdentity + ' model for the ' + attributeName + ' attribute.');
      sails.log.debug('In the next version of Sails, model instance methods will be removed. You should move the logic from the instance method into a reusable helper function.');
    }
  });
