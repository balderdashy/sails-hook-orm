...
// // ^^TODO: since this no longer seems to be used anywhere, verify that we're still gracefully handling this error, and then if so, we can delete this file
...
module.exports = function validateModelDef (originalModelDef, modelIdentity, hook, sails) {
  ...
  if (!_.isUndefined(normalizedModelDef.autoCreatedAt) || !_.isUndefined(normalizedModelDef.autoUpdatedAt) || !_.isUndefined(normalizedModelDef.autoPK)) {
    throw new Error(
                    ...
                    'see http://next.sailsjs.com/docs/concepts/models-and-orm/model-settings#?primarykey.\n'+
                    '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n');
  }
...
//
  ...
  if (normalizedDatastoreConfig.adapter === 'sails-mongo') {
    ...
    if (normalizedModelDef.datastore !== 'default' || originalModelDef.attributes[normalizedModelDef.primaryKey]) {
      var primaryKeyAttr = normalizedModelDef.attributes[normalizedModelDef.primaryKey];
      if (primaryKeyAttr.autoIncrement || primaryKeyAttr.type !== 'string' || primaryKeyAttr.columnName !== '_id') {
        ...
        sails.log.debug('and `type: \'string\', and must _not_ have `autoIncrement: true`.');
