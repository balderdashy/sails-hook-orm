...
module.exports = function validateModelDef (originalModelDef, modelIdentity, hook, sails) {
  ...
  if (!util.isFunction(sails.config.models.attributes) && !util.isArray(sails.config.models.attributes) && util.isObject(sails.config.models.attributes)) {
  ...
  if (!util.isFunction(originalModelDef.attributes) && !util.isArray(originalModelDef.attributes) && util.isObject(originalModelDef.attributes)) {
  ...
  if (!util.isArray(normalizedModelDef.connection)) {
  ...
  // Validate & normalize the datastore configuration referenced by this model, then save it back to the same property on the model.
  // (If datastore config is invalid, this throws a fatal error.)
  normalizedModelDef.connection[0] = validateDatastoreConfig(normalizedModelDef.connection[0], modelIdentity, hook, sails);
  ...
  if (util.isUndefined(normalizedModelDef.schema)) {
    if (!util.isUndefined(normalizedModelDef.connection[0].schema)) {
      normalizedModelDef.schema = normalizedModelDef.connection[0].schema;
    }
  }
