...
module.exports = function validateModelDef (originalModelDef, modelIdentity, hook, sails) {
  ...
  // If there's a top-level `autoCreatedAt`, `autoUpdatedAt` or `autoPK`, bail out.
  ...
  _.each(normalizedModelDef.attributes, function updateProperties (val, attributeName) {
    ...
    if (typeMapping[val.type]) {
      ...
      sails.log.debug('Please update to a known type (changing to `' + typeMapping[val.type] + '` for now) and set the `columnType` property');
      sails.log.debug('if you wish to use a specific column type supported by your database.');
